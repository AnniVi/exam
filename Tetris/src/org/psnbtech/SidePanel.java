...
public class SidePanel extends JPanel {
	 ...
	 */
	private static final int SQUARE_CENTER_X = 135;
	
	/**
	 * The center y of the next piece preview box.
	 */
	private static final int SQUARE_CENTER_Y = 70;
	...
	/**
	 * Количество пикселей, используемых на небольшом экране (обычно используется для категорий)
	 */
	private static final int SMALL_INSET = 20;
	
	/**
	 * Количество пикселей на больших вставках
	 */
	private static final int LARGE_INSET = 40;
	
	/**
	 *
	 * Координата y категории статистики
	 */
	private static final int STATS_INSET = 165;
	
	/**
	 * Координата y категории элементов управления
	 */
	 ...
	 * Маленький шрифт
	 */
	private static final Font SMALL_FONT = new Font("Tahoma", Font.BOLD, 10);
	
	/**
	 * The large font.
	 */
	private static final Font LARGE_FONT = new Font("Tahoma", Font.BOLD, 12);
	...
	public void paintComponent(Graphics g) {
		...
		g.drawString("Статистика", SMALL_INSET, offset = STATS_INSET);
		g.setFont(SMALL_FONT);
		g.drawString("Уровень: " + tetris.getLevel(), LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Счет: " + tetris.getScore(), LARGE_INSET, offset += TEXT_STRIDE);
		
		/*
		 * Draw the "Controls" category.
		 */
		g.setFont(LARGE_FONT);
		g.drawString("Управление", SMALL_INSET, offset = CONTROLS_INSET);
		g.setFont(SMALL_FONT);
		g.drawString("A - Сдвиг влево", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("D - Сдвиг вправо", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Q - Поворот противчасовой", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("E - Поворот почасовой", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("S - Вниз", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("P - Пауза", LARGE_INSET, offset += TEXT_STRIDE);
		
		/*
		 * Draw the next piece preview box.
		 */
		g.setFont(LARGE_FONT);
		g.drawString("Next:", SMALL_INSET, 50);
